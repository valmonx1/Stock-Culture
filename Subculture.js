...
function renderTable() {
  ...
  tbody.innerHTML = filtered.map((record) => `<tr>
    <td><select class="action-select" data-record-action="${record.id}" aria-label="Actions for ${record.name}"><option value="">Action</option><option value="detail">View details</option><option value="acceptance">Acceptance testing</option><option value="storage">Storage location</option><option value="passages">Passage log</option><option value="usage">Usage / disposal</option></select></td>
    <td><strong>${record.id}</strong></td>
    <td><strong>${record.name}</strong><small>${record.passage === "P0" ? "Original reference" : "Reference stock"}</small></td>
    <td>${record.code}</td><td>${record.batch}</td><td>${record.manufacturer}</td><td>${record.registered}</td><td>${record.purchase}</td><td>${record.received}</td><td>${record.storage}</td><td>${record.location}</td><td>${statusBadge(record.status)}</td>
  </tr>`).join("");
...
function handleRegister(event) {
  ...
  const trailEvents = (() => { try { return JSON.parse(localStorage.getItem('qc-trail-events') || '[]'); } catch { return []; } })();
  trailEvents.push({ recordId: new URLSearchParams(window.location.search).get('record'), process: 'Process Subculture', status: 'Completed', date: data.get('subcultureDate'), user: data.get('subcultureBy') || '—', update: 'Working stock prepared.', details: [`Source vial: ${data.get('sourceTube')}`, `Passage number: ${data.get('newPassage')}`, `Working slant: ${data.get('workingSlant') || '—'}`] });
  localStorage.setItem('qc-trail-events', JSON.stringify(trailEvents));
...
function ensureSubcultureView() {
  ...
  usageView.insertAdjacentHTML("beforebegin", `<section class="view" id="view-subculture" data-view-panel="subculture">
    ...
    <form id="subculture-form" novalidate><div class="form-grid form-grid-top">
      <section class="card form-section"><div class="section-heading"><div class="section-number">01</div><div><span class="eyebrow">Traceable source</span><h2>Select source stock</h2><p>Only accepted, available stock can be used as the parent culture.</p></div><span class="result-pill result-pill-pending">Source required</span></div><div class="form-fields two-columns">
        ...
        <label class="field"><span>Source tube / (Stock Culture Vial) <em>*</em></span><select name="sourceTube"><option value="">Select vial</option><option>V1</option><option>V2</option><option>V3</option><option>V4</option><option>V5</option><option>V6</option><option>V7</option><option>V8</option><option>V9</option><option>V10</option><option>V11</option><option>V12</option></select></label><label class="field"><span>New passage number <em>*</em></span><select name="newPassage"><option value="">Select passage</option><option>P2 · Working stock</option><option>P3 · Working stock</option><option>P4 · Working stock</option></select></label><label class="field"><span>Purpose <em>*</em></span><select name="subculturePurpose"><option value="">Select purpose</option><option>Routine QC working stock</option><option>Replacement stock</option><option>Method verification</option><option>Proficiency testing preparation</option></select></label><label class="field"><span>Date started <em>*</em></span><input name="subcultureDate" type="date" /></label>
      </div></section>
  ...
  const sourceTubeField = document.querySelector('#subculture-form select[name="sourceTube"]')?.closest('.field');
  if (sourceTubeField) sourceTubeField.insertAdjacentHTML('afterend', '<label class="field"><span>Number of working slant prepared</span><select name="workingSlant"><option value="">Select number</option><option>1</option><option>2</option><option>3</option><option>4</option><option>5</option></select></label>');
  const passageField = document.querySelector('#subculture-form select[name="newPassage"]');
  if (passageField) {
    const passageLabel = passageField.closest('.field')?.querySelector('span');
    if (passageLabel) passageLabel.firstChild.textContent = 'Passage Number ';
    passageField.innerHTML = '<option value="">Select passage</option><option>1</option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option>';
  }
  document.querySelector('#subculture-form [name="subculturePurpose"]')?.closest('.field')?.remove();
  const datePerformedLabel = document.querySelector('#subculture-form [name="subcultureDate"]')?.closest('.field')?.querySelector('span');
  if (datePerformedLabel && datePerformedLabel.firstChild) datePerformedLabel.firstChild.textContent = 'Date performed ';
  const sourceOrganism = document.querySelector('#subculture-form select[name="sourceOrganism"]');
  if (sourceOrganism) {
    sourceOrganism.closest('.field')?.classList.remove('field-span-2');
    [...sourceOrganism.options].forEach((option) => { option.textContent = option.textContent.replace(/\s+ATCC\s+\d+.*$/i, ''); });
    sourceOrganism.closest('.field')?.insertAdjacentHTML('afterend', '<label class="field"><span>ATCC</span><select name="sourceAtcc"><option value="">Select ATCC</option><option>ATCC 25922</option><option>ATCC 29213</option><option>ATCC 29212</option><option>ATCC 27853</option></select></label>');
    const recordId = new URLSearchParams(window.location.search).get('record');
    let selectedRecord = null;
    try { const records = JSON.parse(localStorage.getItem('stock-culture-records') || '[]'); selectedRecord = records.find((record) => record.id === recordId) || records[0]; } catch (error) { selectedRecord = null; }
    if (!selectedRecord) selectedRecord = seedRecords.find((record) => record.id === recordId) || seedRecords[0];
    const organismMatch = String(selectedRecord?.name || '').match(/^(.*?)\s+ATCC\s+(\d+)/i);
    const sourceInput = document.createElement('input'); sourceInput.className = 'dimmed-input'; sourceInput.name = 'sourceOrganism'; sourceInput.type = 'text'; sourceInput.value = organismMatch ? organismMatch[1] : ''; sourceInput.placeholder = 'Selected source organism'; sourceInput.readOnly = true;
    const atccSelect = document.querySelector('#subculture-form select[name="sourceAtcc"]');
    const atccInput = document.createElement('input'); atccInput.className = 'dimmed-input'; atccInput.name = 'sourceAtcc'; atccInput.type = 'text'; atccInput.value = organismMatch ? `ATCC ${organismMatch[2]}` : ''; atccInput.placeholder = 'Selected ATCC'; atccInput.readOnly = true;
    [sourceInput, atccInput].forEach((input) => { input.style.background = '#eef2f5'; input.style.color = '#7f90a1'; input.style.cursor = 'not-allowed'; });
    sourceOrganism.replaceWith(sourceInput); atccSelect?.replaceWith(atccInput);
  }
  ...
  const closeSubculture = document.querySelector('#subculture-form button[data-view="passages"]');
  if (closeSubculture) {
    closeSubculture.removeAttribute('data-view');
    closeSubculture.id = 'close-subculture';
    closeSubculture.addEventListener('click', () => { window.location.href = 'index.html'; });
  }
  document.querySelector('#subculture-form .form-grid-bottom')?.remove();
  document.querySelector('#view-subculture .stepper')?.remove();
  const subcultureTopGrid = document.querySelector('#subculture-form .form-grid-top');
  const subcultureSections = subcultureTopGrid?.querySelectorAll('.form-section');
  if (subcultureTopGrid && subcultureSections?.[1]) {
    subcultureTopGrid.classList.add('subculture-sequential');
    subcultureSections[1].hidden = true;
    subcultureSections[0].querySelector('.subculture-proceed-actions')?.remove();
    const performedDate = subcultureSections[0].querySelector('[name="subcultureDate"]');
    if (performedDate) {
      performedDate.outerHTML = '<button class="button button-primary subculture-process-button" type="button" id="process-subculture">Process subculture <span class="button-symbol">→</span></button>';
    }
    const prepareFields = subcultureSections[1].querySelector('.form-fields');
    const workingSlantSelect = document.querySelector('#subculture-form [name="workingSlant"]');
    if (prepareFields && workingSlantSelect) {
      const prepStyle = document.createElement('style');
      prepStyle.textContent = '.working-prep-table{margin-top:18px;overflow-x:auto}.working-prep-table table{width:100%;min-width:900px;border-collapse:collapse}.working-prep-table th{padding:10px 8px;background:#f4f8fc;color:#7b8ea3;font-size:11px;text-align:left;text-transform:uppercase;letter-spacing:.4px}.working-prep-table td{padding:8px;border-bottom:1px solid #e2eaf2}.working-prep-table input,.working-prep-table select{width:100%;min-height:40px;padding:9px 10px;border:1px solid #d4e0ec;border-radius:8px;background:#fff;color:#526a84;font:inherit;font-size:12px}.working-prep-table input:focus,.working-prep-table select:focus{outline:none;border-color:#4b9be8;box-shadow:0 0 0 3px #e6f2ff}.is-hidden{display:none!important}';
      document.head.appendChild(prepStyle);
      prepareFields.insertAdjacentHTML('afterend', '<div id="working-prep-table" class="working-prep-table is-hidden"><table><thead><tr><th>Media / plate</th><th>Media Lot Number</th><th>Incubation condition</th><th>Incubation duration</th><th>Number of tube/loops</th><th>Prepared by</th></tr></thead><tbody id="working-prep-rows"></tbody></table></div>');
      const renderPrepareTable = () => {
        const count = Number(workingSlantSelect.value || 0);
        const table = document.getElementById('working-prep-table');
        const rows = document.getElementById('working-prep-rows');
        if (!table || !rows) return;
        table.classList.toggle('is-hidden', count === 0);
        prepareFields.classList.toggle('is-hidden', count > 0);
        const media = '<option value="">Select medium</option><option>Blood agar plate</option><option>Mueller-Hinton agar</option><option>MacConkey agar</option><option>Sabouraud dextrose agar</option>';
        const incubation = '<option value="">Select condition</option><option>35 ± 2 °C · Ambient air</option><option>35 ± 2 °C · 5% CO₂</option><option>30 ± 2 °C · Ambient air</option><option>25 ± 2 °C · Ambient air</option>';
        const users = '<option value="">Select user</option><option>Sarah A.</option><option>Dr. Amir R.</option><option>Nurul H.</option>';
        rows.innerHTML = Array.from({ length: count }, (_, index) => `<tr class="working-prep-row"><td><select name="subcultureMedia_${index + 1}">${media}</select></td><td><input name="subcultureMediaLot_${index + 1}" placeholder="Lot number" /></td><td><select name="incubation_${index + 1}">${incubation}</select></td><td><input name="incubationDuration_${index + 1}" placeholder="e.g. 18–24 hours" /></td><td><input name="subcultureQuantity_${index + 1}" type="number" min="1" placeholder="e.g. 10" /></td><td><select name="subcultureBy_${index + 1}">${users}</select></td></tr>`).join('');
      };
      workingSlantSelect.addEventListener('change', renderPrepareTable);
    }
  }
}
...
function handleSubculture(event) {
  event.preventDefault();
  const form = event.currentTarget;
  const requiredNames = ["sourceOrganism", "sourceTube", "newPassage", "subcultureDate"];
  const missing = requiredNames.map((name) => form.elements[name]).filter((field) => field && !String(field.value).trim());
  const prepRows = [...form.querySelectorAll('.working-prep-row')];
  if (prepRows.length) {
    prepRows.forEach((row) => row.querySelectorAll('select, input').forEach((field) => { if (/subcultureMedia_|incubation_|subcultureQuantity_|subcultureBy_/.test(field.name) && !String(field.value).trim()) missing.push(field); }));
  } else {
    ["subcultureMedia", "incubation", "subcultureQuantity", "subcultureBy"].map((name) => form.elements[name]).filter(Boolean).forEach((field) => { if (!String(field.value).trim()) missing.push(field); });
  }
  if (missing.length) {
    missing[0].focus();
...
document.getElementById("save-subculture-draft").addEventListener("click", () => {
  const form = document.getElementById("subculture-form");
  const firstStepNames = ["sourceOrganism", "sourceTube", "newPassage", "subcultureDate"];
  const missing = firstStepNames.map((name) => form.elements[name]).filter((field) => field && !String(field.value).trim());
  const secondStep = form.querySelector('.form-grid-top .form-section:nth-child(2)');
  if (secondStep?.hidden) {
    if (missing.length) { missing[0].focus(); showToast(`Lengkapkan ${missing.length} medan sumber sebelum meneruskan.`); return; }
    secondStep.hidden = false;
    showToast("Maklumat sumber disimpan. Sila lengkapkan Prepare subculture.");
    secondStep.scrollIntoView({ behavior: "smooth", block: "start" });
    return;
  }
  showToast("Subculture draft saved locally for this mockup.");
});
document.getElementById("process-subculture")?.addEventListener("click", () => document.getElementById("save-subculture-draft").click());
setupDetailTabs();
renderTable();
updateDetail(state.selectedId);
setView("subculture");
const PAGE_LINKS = { dashboard: "Main_Page.html", register: "Register_QC.html", detail: "Organism_Detail.html", acceptance: "Acceptance_Testing.html", storage: "Storage_Map.html", passages: "Passage_Log.html", subculture: "Subculture.html", usage: "Usage_Disposal.html" };
document.addEventListener("click", (event) => { const trigger = event.target.closest("[data-view]"); if (trigger && PAGE_LINKS[trigger.dataset.view]) window.location.href = PAGE_LINKS[trigger.dataset.view]; });
document.addEventListener("change", (event) => { const trigger = event.target.closest("[data-record-action]"); if (trigger && PAGE_LINKS[trigger.value]) window.location.href = PAGE_LINKS[trigger.value]; });
