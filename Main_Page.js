...
function renderTable() {
  ...
  tbody.innerHTML = filtered.map((record) => `<tr>
    <td><select class="action-select" data-record-action="${record.id}" aria-label="Actions for ${record.name}"><option value="" selected disabled hidden>Action</option><option value="monthly-checking">Culture Checking</option><option value="subculture">Process subculture</option><option value="storage">Storage</option><option value="qc-trail">QC Trail</option><option value="test">Test</option></select></td>
...
setView("dashboard");
const PAGE_LINKS = { dashboard: "Main_Page.html", register: "Register_QC.html", detail: "Organism_Detail.html", acceptance: "Acceptance_Testing.html", storage: "Storage.html", passages: "Passage_Log.html", subculture: "Subculture.html", "monthly-checking": "Monthly_Checking.html", "qc-trail": "QC_Trail.html", test: "Entry.html", usage: "Usage_Disposal.html" };
...
/* Capture navigation before the legacy in-page view handler runs. */
document.addEventListener("click", (event) => { const trigger = event.target.closest("[data-view]"); if (trigger && PAGE_LINKS[trigger.dataset.view]) { event.preventDefault(); event.stopPropagation(); window.location.href = PAGE_LINKS[trigger.dataset.view]; } }, true);
document.addEventListener("change", (event) => { const trigger = event.target.closest("[data-record-action]"); if (!trigger) return; if (PAGE_LINKS[trigger.value]) { event.preventDefault(); event.stopPropagation(); const withRecord = ["qc-trail", "monthly-checking", "subculture", "storage", "test"].includes(trigger.value); window.location.href = PAGE_LINKS[trigger.value] + (withRecord ? `?record=${encodeURIComponent(trigger.dataset.recordAction || "")}` : ""); } }, true);
document.querySelectorAll('[data-view-panel]:not(#view-dashboard)').forEach((panel) => panel.remove());
