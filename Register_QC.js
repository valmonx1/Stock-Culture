...
function showSuccessModal(message, form) {
  ...
  backdrop.innerHTML = `<div class="success-modal" role="dialog" aria-modal="true" aria-labelledby="success-modal-title"><div class="success-modal-icon">✓</div><h2 id="success-modal-title">Success</h2><p>${message}</p><div class="success-modal-actions"><button class="button button-secondary" type="button" data-success-close>Close</button><button class="button button-primary" type="button" data-success-ok>Okay</button></div></div>`;
  document.body.appendChild(backdrop);
  backdrop.querySelector("[data-success-close]").addEventListener("click", () => {
    form.reset();
    const selectedFile = document.getElementById("selected-file");
    if (selectedFile) selectedFile.classList.add("is-hidden");
    backdrop.remove();
  });
  backdrop.querySelector("[data-success-ok]").addEventListener("click", () => { window.location.href = "Main_Page.html"; });
}
...
function handleRegister(event) {
  ...
  showSuccessModal(data.get("releaseToStorage") ? "QC organism registered and released to storage." : "QC organism registered as draft.", form);
}
